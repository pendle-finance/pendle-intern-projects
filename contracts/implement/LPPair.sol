...
contract LPPair is ERC20, ILPPair {
  ...
  function mint(address to) public returns (uint256 liquidity) {
    ...
    if (totalBalance == 0) {
      liquidity = Math.sqrt(amount1.mul(amount0));
    } else {
      liquidity = Math.sqrt(balance0.mul(balance1)).sub(Math.sqrt(_reserve0.mul(_reserve1)));
    }
  ...
  function burn(address to,uint256 liquidity) external returns (uint256 amount0, uint256 amount1) {
    require(msg.sender == to || allowBalances[to][msg.sender] >= liquidity,"Invalid operation burn");
