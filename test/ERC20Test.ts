...
import { ERC20Public } from "../typechain";

xdescribe("ERC20Test", () => {
  const [admin, alice, bob, charlie] = waffle.provider.getWallets();
  let globalSnapshotId;
  let snapshotId;
  let erc20: ERC20Public;

  before(async () => {
    globalSnapshotId = await evm_snapshot();

    erc20 = await deploy<ERC20Public>('ERC20Public', ["testToken", "test"]);
  ...
  it("Mint and burn works", async () => {
    ...
    await erc20.mintPublic(alice.address, BigNumber.from(10).pow(18));

    const afterMintBalance = await erc20.balanceOf(alice.address);
    expect(afterMintBalance).to.be.eq(BigNumber.from(10).pow(18));

    await erc20.burnPublic(alice.address, BigNumber.from(10).pow(18));
  ...
  it('transfer works', async () => {
    await erc20.mintPublic(admin.address, BigNumber.from(10).pow(18));
  ...
  it('transferFrom works', async () => {
    await erc20.mintPublic(bob.address, BigNumber.from(10).pow(18));
  ...
  it('transferring more than available', async () => {
    await erc20.mintPublic(admin.address, 200);
  ...
  it('transferFrom more than available', async () => {
    await erc20.mintPublic(bob.address, 400);
  ...
  it('Burning more than available', async () => {
    await erc20.mintPublic(bob.address, 400);
    await expect(erc20.burnPublic(bob.address, 401)).to.be.revertedWith("Insufficient balance");
  });
  ...
  it('Zero address reverts', async () => {
    ...
    await expect(erc20.burnPublic(ZERO_ADDRESS, 401)).to.be.revertedWith("Invalid address");
  });
  ...
  it('transferFrom() emits transfer', async () => {
    await erc20.mintPublic(bob.address, 200);
    await erc20.connect(bob).approve(admin.address, 200);
  ...
  it('mint() and burn() emits Mint and Burn', async () => {
    await expect(erc20.mintPublic(bob.address, 200)).to.emit(erc20, 'Mint').withArgs(bob.address, 200);
    await expect(erc20.burnPublic(bob.address, 200)).to.emit(erc20, 'Burn').withArgs(bob.address, 200);
  })
